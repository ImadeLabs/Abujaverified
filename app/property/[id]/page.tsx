...
export default async function PropertyDetail({ params }: { params: { id: string } }) {
  // 1. Fetch the specific property from Neon
      ...
      <div className="row g-5">
        ...
        <div className="col-lg-8">
          <div className="mb-4">
            ...
            <p className="text-muted"><i className="bi bi-geo-alt-fill me-2"></i>{property.location}</p>
          </div>

          {/* Main Photo Gallery (Scrollable) */}
          <div className="d-flex gap-2 overflow-auto pb-3 mb-4" style={{scrollbarWidth: 'none'}}>
            {property.images?.map((url: string, i: number) => (
              ...
              />
            )) || <img src={property.imageUrl} className="w-100 rounded-4" />}
          </div>
        ...
        {/* RIGHT COLUMN: Contact & Audit Status */}
        <div className="col-lg-4">
          <div className="card border-0 shadow-lg rounded-4 p-4 sticky-top" style={{ top: '100px' }}>
            <h2 className="fw-bold text-success mb-4">{property.price}</h2>
            
            <div className="mb-4 p-3 bg-light rounded-3">
              <div className="d-flex align-items-center mb-2">
                <span className={`badge ${property.status === 'TIER_2_VERIFIED' ? 'bg-success' : 'bg-warning text-dark'} me-2`}>
                  {property.status === 'TIER_2_VERIFIED' ? '✓' : '!'}
                </span>
